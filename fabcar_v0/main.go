...
/*
	fabcar
		1. create(carID,make,model,colour,owner)
		2. transfer(carID,newOwner)
		3. query(carID)
		4. queryByOwner(owner)
		5. purge(carID)
*/
...
func create(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// args : carID , make , model , colour , owner
	if len(args) != 5 {
		return invalidNumberOfArgument(args, 5)
	}
